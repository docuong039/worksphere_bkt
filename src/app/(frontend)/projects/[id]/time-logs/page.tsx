...
export default function ProjectTimeLogsPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchTimeLogs = async () => {
        ...
        try {
            ...
            // Mock data for fallback - US-MNG-01-08
            setLogs([
    ...
    return (
        <AppLayout>
            <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in duration-700" data-testid="project-time-logs-page">
                {/* Header */}
                    ...
                    <div>
                        <Button variant="ghost" asChild className="-ml-4 mb-2 text-slate-500 hover:text-slate-900">
                            <Link href={`/projects/${projectId}/overview`}>
                                <ChevronLeft className="mr-2 h-4 w-4" /> Quay lại Tổng quan
                            </Link>
                        </Button>
                        <h1 className="text-3xl font-black tracking-tight text-slate-900" data-testid="time-logs-title">
                            Nhật ký thời gian Dự án
                        </h1>
                        <p className="text-slate-500 text-sm mt-1 font-medium italic">
                            Xem chi tiết thời gian làm việc của các thành viên (US-MNG-01-08)
                        </p>
                    </div>
                    ...
                    <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden" data-testid="time-logs-table">
                        ...
                        {filteredLogs.length === 0 && (
                            <div className="p-20 text-center text-slate-300">
                                ...
                                <p className="font-bold text-lg">Không có nhật ký thời gian nào.</p>
                            </div>
                        )}
                    </div>
                )}
